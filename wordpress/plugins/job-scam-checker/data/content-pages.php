...
$pages['privacy'] = $page( 'Privacy Policy', 'privacy-policy', 'Privacy Policy | Job Scam Checker', 'How Job Scam Checker processes pasted text, rate-limit identifiers, WordPress data and optional third-party integrations.', 'legal',
    '<p><strong>Last reviewed:</strong> Phase 5 release.</p><p>This policy describes the custom Job Scam Checker functionality. The hosting provider and WordPress installation may maintain standard security or server logs outside the custom checker code.</p><h2>Pasted messages</h2><p>The checker sends pasted text to this website’s own WordPress endpoint for immediate rule-based analysis. The custom code does not permanently store the message, add it to statistics, place it in a URL, return it in the result or send it to an AI provider. Remove sensitive information before submitting.</p><h2>Optional anonymous statistics</h2><p>The administrator may enable daily aggregate counters for total checks, risk levels and detected warning-rule identifiers. Optional follow-up selections may count how an offer arrived, whether money was requested and the selected payment purpose. These counters contain no pasted text, names, phone numbers, email addresses, full URLs, identity documents, banking details, IP addresses or visitor profiles. They cannot be traced back to an individual checker result. If disabled, the checker continues to work and follow-up questions are not shown. Aggregates are retained for the period selected by the administrator.</p><h2>Rate limiting and duplicate prevention</h2><p>To reduce abuse, the checker temporarily stores a request counter under a salted one-way identifier derived from the connecting network address. The raw address and message are not stored in that transient by the custom code. A short-lived random one-use token prevents repeated follow-up counting; it contains no message or visitor identity. These temporary values expire automatically.</p><h2>Pages and administration</h2><p>WordPress stores site pages, editable detection rules, aggregate privacy settings and normal administrator settings. Visitors do not need an account to use the checker.</p><h2>Analytics and advertising</h2><p>The current custom implementation does not require Google Analytics or AdSense. Empty integration placeholders do not load advertisements or trackers. If analytics or advertising is enabled later, this policy and any consent controls should be updated before deployment to describe the actual providers and cookies.</p><h2>Contact and retention</h2><p>No public contact channel is fabricated. The site operator should add accurate contact details and jurisdiction-specific privacy information before production publication. Removing the plugin deletes its rule and aggregate tables and options but preserves administrator-edited pages.</p>', array( 'terms', 'disclaimer', 'about' ) );
$pages['privacy']['upgrade_from'] = array( '<p><strong>Last reviewed:</strong> Phase 4 release.</p><p>This policy describes the custom Job Scam Checker functionality. The hosting provider and WordPress installation may maintain standard security or server logs outside the custom checker code.</p><h2>Pasted messages</h2><p>The checker sends pasted text to this website’s own WordPress endpoint for immediate rule-based analysis. The custom code does not permanently store the message, add it to statistics, place it in a URL, return it in the result or send it to an AI provider. Remove sensitive information before submitting.</p><h2>Rate limiting</h2><p>To reduce abuse, the checker temporarily stores a request counter under a salted one-way identifier derived from the connecting network address. The raw address and message are not stored in that transient by the custom code. The counter expires automatically.</p><h2>Pages and administration</h2><p>WordPress stores site pages, editable detection rules and normal administrator settings. Visitors do not need an account to use the checker.</p><h2>Analytics and advertising</h2><p>The current custom implementation does not require Google Analytics or AdSense. Empty integration placeholders do not load advertisements or trackers. If analytics or advertising is enabled later, this policy and any consent controls should be updated before deployment to describe the actual providers and cookies.</p><h2>Contact and retention</h2><p>No public contact channel is fabricated. The site operator should add accurate contact details and jurisdiction-specific privacy information before production publication. Removing the plugin deletes its rule table and options but preserves administrator-edited pages.</p>' );
